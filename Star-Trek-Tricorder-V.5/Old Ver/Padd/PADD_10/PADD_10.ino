...
void loop() {//BEGINS MAIN PART OF THE PROGRAM
  ...
  (f);//PRINTS THE F IN SERIAL MONITER
  lcd.setCursor(7,1);//RESETS THE CURSOR TO THE SECOND ROW
  lcd.print(h); //PRINTS THE HUMIDITY
  ("HUMIDITY");//WRITES THE LABEL FOR THE HUMIDITY ON THE SERIAL MONITER
  (h);//PRINTS THE HUMIDITY IN THE SERIAL MONITER
  if (h>40) {//WARNING FOR HIGH HUMIDITY
    ("||RED ALERT||");
    ("BIOCONTAMINATION");
    ("EVACUATE ZONE: HIGH HUMIDITY");
    lcd.print("||RED ALERT||");
    delay(500);
    ("BIOCONTAMINATION");
    delay(500);
  ...
  if (f>100) {//WARNING FOR TOO HOT 
    ("||RED ALERT||");
    ("TIME DISTORTIONS");
    ("EVACUATE ZONE: HIGH TEMPERATURE");
    lcd.print("||RED ALERT||");
    delay(500);
    ("TIME DISTORTIONS");
    delay(500);
...
//---------------------------------------------------------------------------------------------
//OBJECTIVE:
//THE OBJECTIVE OF THIS PROJECT IS TO CREATE A STAR TREK PADD AS REAL AS POSSIBLE.
...
//-HOOK UP TO A BATTERY PACK FOR USE SO THAT THE PADD CAN BE MORE PORTABLE
